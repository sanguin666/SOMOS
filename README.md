...
Every place's home page opens with up to four small tiles, its **badges** (`src/poi-badges`, `pois/:poiId/badges`): next Mass, office open or closed, next confessions and a campaign's progress, all worked out on the phone (`app/src/utils/badges.ts`), plus short messages the office writes (40 characters, an important one in orange, an optional last day and a module a tap opens). The dashboard's Home page switches them on and off and sets their order; a place that never touched them shows the next Mass and the office hours.
